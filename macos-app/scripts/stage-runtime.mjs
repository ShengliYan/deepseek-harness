 ...
 *   4. pnpm --filter @deepseek-ai/dsh-desktop-runtime deploy --prod --legacy --config.node-linker=hoisted runtime-stage
...
import { sydneyAppVersion } from './app-version.mjs'
...
// The release-family glob `apps/*/package.json` sweeps this deploy root into
// the dsh family, so it must carry an @deepseek-ai name and the family version
// (the workspace root manifest's), or release verification rejects the tree.
const manifestName = '@deepseek-ai/dsh-desktop-runtime'
...
function generateManifest() {
  ...
  const familyVersion = JSON.parse(readFileSync(join(repoRoot, 'package.json'), 'utf8')).version
  const manifest = {
    name: manifestName,
  ...
  try {
    while (Date.now() < deadline) {
      ...
      if (launchUrl !== null) {
        ...
        if (ok && look.ready) {
          log('smoke', `server ready on :${smokePort} (manifest rev ${look.rev})`)
          return
        }
...
// App version is the Sydney clock (YYYY.MM.DD.HHmm) so About / the update
// dialog show when this build was cut. build-id still suffixes a unique
// stamp so two dist runs in the same minute remain distinguishable.
const version = sydneyAppVersion()
const buildId = `${version}.${Date.now().toString(36)}`
mkdirSync(join(macosAppDir, 'build'), { recursive: true })
writeFileSync(join(macosAppDir, 'build', 'app-version'), `${version}\n`)
...
const updateConfigPath = join(macosAppDir, 'build', 'update-config.json')
const updateConfigLocalPath = join(macosAppDir, 'build', 'update-config.local.json')
const updateConfigPackagedPath = join(macosAppDir, 'build', 'update-config.packaged.json')
const updateConfig = JSON.parse(readFileSync(updateConfigPath, 'utf8'))
if (typeof updateConfig.feedUrl === 'string' && updateConfig.feedUrl.startsWith('file://')) {
  fail('committed update-config.json must not use a file:// feedUrl; put the local loop URL in build/update-config.local.json')
}
const updateConfigLocal = existsSync(updateConfigLocalPath)
  ? JSON.parse(readFileSync(updateConfigLocalPath, 'utf8'))
  : {}
writeFileSync(updateConfigPackagedPath, `${JSON.stringify({ ...updateConfig, ...updateConfigLocal }, null, 2)}\n`)
log('done', `${stageDir} (version ${version} buildId ${buildId})`)
