...
**在会话列表里把 fork 子会话嵌套到源会话之下。**否决：同级行模型（两行都独立可选、可搜索、可拖拽排序）是既定决策（[2026-07-27-web-session-fork-actions.md](2026-07-27-web-session-fork-actions.md)）；标题栏面包屑在不改变列表归属的前提下补上谱系导航。
