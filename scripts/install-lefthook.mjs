...
async function main() {
  ...
  try {
    ...
    // `lefthook` is a devDependency, so production-only installs (such as the
    // `pnpm install --production` pnpm runs for verify-deps-before-run) do not
    // include it. There is nothing to install in that case, so skip quietly.
    if (errorCode(error) === 'ERR_MODULE_NOT_FOUND') return
    throw error
  }
