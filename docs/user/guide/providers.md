...
- **A custom provider's reasoning model offers no effort levels** — A hand-declared model carries no reasoning capability, so no effort selector appears for it. Declare the levels the provider documents under that model's `reasoningEfforts` in `$DSH_HOME/settings.yaml` — each key a selectable level, its value the wire spelling, as in `low: low`, `high: high`, `max: max`. The [`dsh-llm-pi-ai` reference](../../../packages/llm/llm-pi-ai/README.md) owns the field contract.
