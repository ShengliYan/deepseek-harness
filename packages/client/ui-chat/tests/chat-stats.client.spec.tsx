...
describe('StatsPills', () => {
  ...
  it('renders the counts reading and usage pill and hides a brand-new empty session', () => {
    ...
    expect(usagePill.map(pill => pill.textContent)).toEqual(['105 tok·Cache hit 90.00%'])
    expect(usagePill[0]!.getAttribute('aria-label')).toBe('105 tok · Cache hit 90.00%')
  ...
  it('keeps the durable usage pill after the visible step window is empty', () => {
    ...
    expect(pills[0]!.textContent).toBe('105 tok·Cache hit 90.00%')
  })
  ...
  it('includes cache writes in the total and the cache-hit denominator', () => {
    ...
    expect(view.getAllByRole('button')[0]!.textContent).toBe('207 tok·Cache hit 45.00%')
  })
