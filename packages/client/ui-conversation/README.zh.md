...
- **assistant 逐消息分页是预留 slot**：设计中已有图稿，尚未实现。已定稿的内容 IconActions 行（复制／时钟／分支）只挂在每个已结束轮次中最后一条带 text 内容的 assistant 下；轮次中间的叙述、纯 Think 节点，以及仍在产出步骤的轮次里的所有节点都不带 chrome。除非该消息同时也是已完成轮次的最后一个 transcript 节点，否则分支保持禁用；启用后，它会 fork 到该轮次末尾，在 client 端递增继承标题并打开子会话。每个持久化用户气泡还带有「回退」按钮（从此处回退并编辑 / Rewind and edit from here）：它以 `rewind: true` fork，子会话只播种该问题所在轮次之前已完成的轮次（问题为第一轮时种子为空），打开子会话并把原文预填进输入框供编辑。会话标题栏显示该 fork 谱系上的 `‹ n/m ›` 版本翻页器——左箭头打开直接父版本，右箭头打开最新直接子版本，单版本会话不渲染。fork 或改名失败时源会话保持选中（[决策](../../../.agents/notes/implemented/bug-fix/2026-08-02-message-fork-actions-require-completed-turn-tail.md)）。
