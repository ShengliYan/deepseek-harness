...
describe('sessions.fork', () => {
  ...
  it('rewinds before the anchored turn: the child seeds only the completed prefix', async () => {
    ...
    const response = await api(ctx).sessions.fork(request({ sessionId: source.id, atSeq: 3, rewind: true }))
    expect(response.result.ok).toBe(true)
    if (!response.result.ok) return
    const child = ctx.sessions.get(response.result.value.sessionId)
    expect(child?.events.map(event => event.type)).toEqual([
  ...
  it('rewinds from the first turn into an empty child', async () => {
    ...
    const response = await api(ctx).sessions.fork(request({ sessionId: source.id, atSeq: 1, rewind: true }))
    expect(response.result.ok).toBe(true)
    if (!response.result.ok) return
    const child = ctx.sessions.get(response.result.value.sessionId)
  ...
  it('rewind without an anchor keeps the last-completed-turn shortcut', async () => {
    ...
    const response = await api(ctx).sessions.fork(request({ sessionId: source.id, rewind: true }))
    expect(response.result.ok).toBe(true)
    if (!response.result.ok) return
    const child = ctx.sessions.get(response.result.value.sessionId)
    expect(child?.events.map(event => event.type)).toEqual([
